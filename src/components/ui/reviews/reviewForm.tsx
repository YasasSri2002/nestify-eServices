...
export default function ReviewForm({ onClose, gigId, providersId }: { onClose: () => void, gigId: string, providersId: string }) {
    ...
    return (
        <div className="flex justify-center items-center inset-0 z-50 fixed w-full "
            onClick={onClose}>

            <div className="grid gap-5 justify-items-center content-center w-dvw max-w-2xl bg-surface-snow p-5 
                 rounded-2xl max-h-[95vh]   mx-4 sm:mx-0"
                ...
                <form onSubmit={submitForm} className="grid content-center gap-4">
                    ...
                    <div className="grid gap-2">
                        ...
                        <textarea name="message" id="client-message"
                            className="w-60 sm:w-100 lg:w-120 h-30 sm:h-50 rounded-2xl px-2 pt-2
                             border focus:outline-0 resize-none"></textarea>

                    </div>

                    <div className="flex justify-center items-center w-full mb-2">
                        <button type="submit"
                            className="rounded-xl bg-accent-600 px-5 py-2 text-white/85 hover:scale-101
                            w-full active:scale-95 active:bg-accent-500 hover:bg-accent-500 shadow-md shadow-accent-600
                            " >
                            Submit
                        </button>
                    </div>
